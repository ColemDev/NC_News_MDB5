...
import { MDBContainer, MDBRow, MDBCol, MDBCard, MDBCardHeader, MDBCardBody, MDBCardText, MDBBtn, MDBIcon, MDBCardFooter } from 'mdb-react-ui-kit';
...
const CommentList = ({loggedInUser, article_id}) => {
  ...
  const DeleteButton = ({comment_id, loggedInUser, author}) => {
    if (loggedInUser === author) {
      return (
        <MDBBtn color='white' className='text-muted' onClick={() => 
...
// navigate to the same page to refresh the article, it's number of votes widget and the comments list.
      ...
      <MDBRow>
        <MDBCol>
          {comments.map((comment) => (
            <MDBCard key={comment.comment_id} author={comment.author}
            className='mb-3 border border-primary rounded-3'>
              ...
              <MDBCardFooter>
                <MDBRow className='row-cols-3 row-cols-md-3'>
                  ...
                  <MDBCol>
                    <DeleteButton comment_id={comment.comment_id} loggedInUser={loggedInUser} author={comment.author} />
                  </MDBCol>
                </MDBRow>
              </MDBCardFooter>
            </MDBCard>
